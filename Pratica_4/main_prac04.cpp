...
void display(void)
{
	...
	view = glm::rotate(view,glm::radians(angulox),glm::vec3(1,0,0));
	view = glm::rotate(view, glm::radians(anguloy), glm::vec3(0, 1, 0));
	view = glm::rotate(view, glm::radians(anguloz), glm::vec3(0, 0, 1));
	...
	model = glm::scale(model, glm::vec3(2, 4.5, 1));
	...
	//Derecha
	//hombro
	model = glm::mat4(1.0f);		// initialize Matrix, Use this matrix for individual models
	...
	glDrawArrays(GL_QUADS, 0, 24);
	//mano
	model = glm::scale(model, glm::vec3(.5, 3.5, 1));
	...
	glDrawArrays(GL_QUADS, 0, 24);
	//mano
	model = glm::scale(model, glm::vec3(.5, 4, 1));
	model = glm::translate(model, glm::vec3(.5, -.5, 0));
	projectionShader.setVec3("aColor", glm::vec3(1.0f, 0.0f, 1.0f));
	projectionShader.setMat4("model", model);
	glDrawArrays(GL_QUADS, 0, 24);
	...
	//Derecho
	//Pie
	model = glm::scale(model, glm::vec3(.25, 3, 1));
	model = glm::translate(model, glm::vec3(-1.5, -.5, 0));
	...
	model = glm::mat4(1.0f);
	model = glm::scale(model, glm::vec3(4, 1, 1));
	model = glm::translate(model, glm::vec3(-.75, -6.5, 0));
	projectionShader.setVec3("aColor", glm::vec3(0.166f, 0.094f, 0.046f));
	projectionShader.setMat4("model", model);
	glDrawArrays(GL_QUADS, 0, 24);
	//Pies
	model = glm::mat4(1.0f);		// initialize Matrix, Use this matrix for individual models
	...
	//glDrawArrays(GL_QUADS, 0, 24);
	//model = glm::mat4(1.0f);		// initialize Matrix, Use this matrix for individual models
	//view = glm::mat4(1.0f);		//Use this matrix for ALL models
	//projection = glm::mat4(1.0f);	//This matrix is for Projection
	//model = glm::translate(model, glm::vec3(-3, 0, 0));
	//model = glm::scale(model, glm::vec3(0.33, 0.25, 0.2));
	//projectionShader.setVec3("aColor", glm::vec3(0.0f, 0.0f, 1.0f));
	//projectionShader.setMat4("model", model);
	//glDrawArrays(GL_QUADS, 0, 24);
	glBindVertexArray(0);
